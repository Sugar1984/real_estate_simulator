...
class real_estate_sym_base:
    def __init__(self, price, closing_costs, term, rate_year, management_fee_month, repair_fund_month, inflation, square_meters, building_ratio, site_ratio):
        ...
        for i in range(1, term + 1):
            principal_payment = -npf.ppmt(rate, i, nper, pv, fv=fv, when=when)
            interest_payment = -npf.ipmt(rate, i, nper, pv, fv=fv, when=when)
            management_fee = management_fee_month * 12 * (1 + inflation) ** (i - 1) * 10000
            repair_fund = repair_fund_month * 12 * (1 + inflation) ** (i - 1) * 10000
            principal -= principal_payment

            bulding_tax = price * building_ratio * 0.014 * (48 - i) / 47 * 10000
